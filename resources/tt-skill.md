...
- Tool: `list_tasks(status="pending", limit=1)`
...
### Block/Unblock Task
- Tool: `edit_task` with action="block" or action="unblock"
- Use when: Waiting on external factor

### List Tasks
- Tool: `list_tasks` with status filter or active filter
- Use: `list_tasks(status="pending")` for pending tasks
- Use: `list_tasks(active=true)` for all active tasks (pending or in_progress)

## Required Workflow - ALWAYS FOLLOW THIS:

1. Start of session: Call `get_current_task` → none? use `list_tasks(status="pending", limit=1)`
...
- Skip marking complete - ALWAYS use `advance_task`
